...
import { Search, Home, Video, Monitor, Layers, Trophy, TrendingUp, Eye, Play } from 'lucide-react';
...
const DesktopBox = () => {
  ...
  const EsportsVidep = [
    { id:"hhlgphVf-1g?list=PLTFsoy_DWCOOvT2L8CzAlgqp0997RZjv2", title: "RETAKE Cinematic - VALORANT", channel:'ValorantEsports', category:'Tactical'},
    { id:"hvtR6Skx7_4", title: "THE MYSTERY OF HEROBRINE IN MINECRAFT", channel:'MrBeast', category:'Tactical'},
    { id:"5bdsq08pG-E", title: "LEVEL 1 VS LEVEL 100 MINECRAFT CANNON", channel:'Minecraft', category:'Tactical'},
    { id:"e_E9W2vsRbQ?list=PLTFsoy_DWCOOvT2L8CzAlgqp0997RZjv2", title: "DUELISTS - VALORANT Cinematic", channel:'ValorantEsports', category:'Tactical'},
  ]

  const Sports = [
    { id:"dLuLdgBHcWs", title:"Cherki's Brilliant Season So Far | Premier League 25/26 Highlights", channel:'PremierLeague', category:'Football' },
    { id:"ZqwttIdH840", title:"Race Highlights | 2026 Australian Grand Prix", channel:'Formula1', category:'F1' },
  ]

  // ── Navigate to the watch page, passing video data via route state ──
  ...
  // Extract base YouTube ID (strip playlist query params) for thumbnail
  const thumbUrl = (id) => {
      ...
      <div className="vcard-thumb">
        ...
        {video.viewers && (
          <div className="viewer-tag">
            <Eye size={12} style={{ marginRight: '5px' }} /> {video.viewers}
          </div>
        )}

        {/* ── Thumbnail image instead of embedded iframe ── */}
        <img
          src={thumbUrl(video.id)}
          alt={video.title}
          style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
          onError={(e) => { e.target.src = `https://img.youtube.com/vi/${video.id.split('?')[0]}/hqdefault.jpg`; }}
        />
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .vcard:hover .play-overlay { background: rgba(0,0,0,0.45); }
      ...
      <aside>
        ...
        <div className={`side-item ${activeTab === 'Home' ? 'active' : ''}`} onClick={() => { setActiveTab('Home'); navigate('../Destop'); }}>
          <Home size={20} /> Home
        </div>
      ...
      <main>
        ...
        <div className="video-grid">
          {EsportsVidep.map((video) => <VideoCard key={video.id} video={video} />)}
        </div>
      </main>
      ...
      <main>
        ...
        <div className="video-grid">
          {Sports.map((video) => <VideoCard key={video.id} video={video} />)}
        </div>
      </main>
    </>
  );
